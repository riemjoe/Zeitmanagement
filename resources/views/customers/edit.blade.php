@extends('layouts.app')
@section('title', 'Kunde bearbeiten: ' . $customer->name)
    ...
    <form method="POST" action="{{ route('customers.update', $customer) }}" class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <textarea name="notes" rows="3"
                      class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('notes', $customer->notes) }}</textarea>
        </div>
