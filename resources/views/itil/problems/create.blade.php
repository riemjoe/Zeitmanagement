@extends('layouts.app')
@section('title', 'Neues Problem')

@section('content')
<div class="max-w-3xl">
<form method="POST" action="{{ route('itil.problems.store') }}" class="space-y-6">
    @csrf
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500">{{ old('description') }}</textarea>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div>
            ...
            <textarea name="root_cause" rows="3"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500">{{ old('root_cause') }}</textarea>
        </div>
        <div>
            <label class="block text-xs font-medium text-gray-600 mb-1">Workaround</label>
            <textarea name="workaround" rows="3"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500">{{ old('workaround') }}</textarea>
        </div>
        <div>
            <label class="block text-xs font-medium text-gray-600 mb-1">Lösung</label>
            <textarea name="resolution" rows="3"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500">{{ old('resolution') }}</textarea>
        </div>
    </div>

    <div class="flex gap-3">
        <button type="submit" class="bg-orange-600 hover:bg-orange-700 text-white text-sm font-medium px-5 py-2 rounded-lg">Problem erstellen</button>
        <a href="{{ route('itil.problems.index') }}" class="text-sm text-gray-500 hover:text-gray-700 px-5 py-2">Abbrechen</a>
    </div>
</form>
</div>
@endsection
