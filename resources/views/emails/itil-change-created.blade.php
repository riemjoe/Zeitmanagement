...
.header { background:#0f172a; padding:28px 32px 24px; border-bottom:3px solid #6366f1; }
...
.badge-standard  { background:#f3f4f6; color:#4b5563; }
...
<div class="wrapper">
  <div class="card">
    <div class="header">
      <div class="header-label">🔄 Neuer Change · Via Webhook</div>
      <div class="header-title">{{ $change->title }}</div>
    </div>
    <div class="body">
      <p class="intro">
        Ein neuer Change wurde über einen Webhook-Endpunkt erstellt.
      </p>

      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-label">Change-ID</div>
          <div class="meta-value" style="font-family:monospace;letter-spacing:.1em;">{{ $change->number }}</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Typ</div>
          <div class="meta-value">
            <span class="badge badge-{{ $change->type }}">{{ $change->type_label }}</span>
          </div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Priorität</div>
          <div class="meta-value">
            <span class="badge badge-{{ $change->priority }}">{{ $change->priority_label }}</span>
          </div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Status</div>
          <div class="meta-value">{{ $change->status_label }}</div>
        </div>
        @if($change->customer)
        <div class="meta-item">
          <div class="meta-label">Kunde</div>
          <div class="meta-value">{{ $change->customer->name }}</div>
        </div>
        @endif
        @if($change->affected_service)
        <div class="meta-item">
          <div class="meta-label">Betroffener Service</div>
          <div class="meta-value">{{ $change->affected_service }}</div>
        </div>
        @endif
        @if($change->planned_start_at)
        <div class="meta-item">
          <div class="meta-label">Geplanter Start</div>
          <div class="meta-value">{{ $change->planned_start_at->format('d.m.Y H:i') }}</div>
        </div>
        @endif
        @if($change->planned_end_at)
        <div class="meta-item">
          <div class="meta-label">Geplantes Ende</div>
          <div class="meta-value">{{ $change->planned_end_at->format('d.m.Y H:i') }}</div>
        </div>
        @endif
      </div>
      ...
      <a href="{{ url('/itil/changes/' . $change->id) }}" class="cta-button">Change im Adminbereich öffnen →</a>
    </div>
    <div class="footer">
      Diese E-Mail wurde automatisch von <strong>{{ config('app.name') }}</strong> versandt.<br>
      Erstellt: {{ $change->created_at->format('d.m.Y H:i') }} Uhr
    </div>
  </div>
</div>
</body>
