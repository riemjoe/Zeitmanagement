...
.header { background:#0f172a; padding:28px 32px 24px; border-bottom:3px solid #ef4444; }
...
.sla-box { background:#fef2f2; border:1px solid #fecaca; border-radius:8px; padding:14px 16px; margin-bottom:24px; }
.sla-box .sla-label { font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:.06em; color:#991b1b; margin-bottom:8px; }
.sla-row { display:flex; justify-content:space-between; font-size:13px; color:#374151; margin-bottom:4px; }
.sla-row strong { color:#111827; }
.description-box { background:#f8fafc; border-left:3px solid #ef4444; border-radius:0 8px 8px 0; padding:14px 16px; font-size:14px; color:#374151; line-height:1.6; margin-bottom:24px; }
...
.cta-button { display:block; background:#ef4444; color:#fff!important; text-decoration:none; text-align:center; font-size:14px; font-weight:600; padding:13px 24px; border-radius:8px; margin-bottom:8px; }
...
<div class="wrapper">
  <div class="card">
    <div class="header">
      <div class="header-label">🚨 Neuer Incident · Via Webhook</div>
      <div class="header-title">{{ $incident->title }}</div>
    </div>
    <div class="body">
      <p class="intro">
        Ein neuer Incident wurde über einen Webhook-Endpunkt erstellt.
      </p>

      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-label">Incident-ID</div>
          <div class="meta-value" style="font-family:monospace;letter-spacing:.1em;">{{ $incident->number }}</div>
        </div>
        <div class="meta-item">
        ...
        <div class="meta-item">
          ...
          <div class="meta-value">{{ $incident->status_label }}</div>
        </div>
        @if($incident->customer)
        <div class="meta-item">
          <div class="meta-label">Kunde</div>
          <div class="meta-value">{{ $incident->customer->name }}</div>
        </div>
        @endif
        @if($incident->affected_service)
        <div class="meta-item">
          <div class="meta-label">Betroffener Service</div>
          <div class="meta-value">{{ $incident->affected_service }}</div>
        </div>
        @endif
        @if($incident->category)
        <div class="meta-item">
          <div class="meta-label">Kategorie</div>
          <div class="meta-value">{{ $incident->category }}</div>
        </div>
        @endif
      </div>
      ...
      <div class="sla-box">
        <div class="sla-label">⏱ SLA-Fristen</div>
        @if($incident->response_due_at)
        <div class="sla-row"><span>Reaktionsfrist</span> <strong>{{ $incident->response_due_at->format('d.m.Y H:i') }} Uhr</strong></div>
        @endif
        @if($incident->resolve_due_at)
        <div class="sla-row"><span>Lösungsfrist</span> <strong>{{ $incident->resolve_due_at->format('d.m.Y H:i') }} Uhr</strong></div>
        @endif
      </div>
      @endif
      ...
      <a href="{{ url('/itil/incidents/' . $incident->id) }}" class="cta-button">Incident im Adminbereich öffnen →</a>
    </div>
    <div class="footer">
      Diese E-Mail wurde automatisch von <strong>{{ config('app.name') }}</strong> versandt.<br>
      Erstellt: {{ $incident->created_at->format('d.m.Y H:i') }} Uhr
    </div>
  </div>
</div>
</body>
