...
@section('header-actions')
    @if($activeTab === 'leistungsberichte')
    <a href="{{ route('leistungsberichte.create') }}"
       class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-lg">+ Neuer Leistungsbericht</a>
    @else
...
{{-- ── Conditional header: nur für Rechnungen & Mahnwesen ──────────────── --}}

{{-- ── Tabs ─────────────────────────────────────────────────────────────────── --}}
<div class="flex items-center gap-1 mb-5 border-b border-gray-200">
    <a href="{{ route('invoices.index') }}"
       class="px-4 py-2 text-sm font-medium rounded-t-lg transition-colors
           {{ $activeTab !== 'mahnwesen' && $activeTab !== 'leistungsberichte' ? 'border-b-2 border-indigo-600 text-indigo-600 bg-white' : 'text-gray-500 hover:text-gray-700' }}">
        Alle Rechnungen
    ...
    <a href="{{ route('invoices.index', ['tab' => 'leistungsberichte']) }}"
       class="px-4 py-2 text-sm font-medium rounded-t-lg transition-colors flex items-center gap-1.5
           {{ $activeTab === 'leistungsberichte' ? 'border-b-2 border-indigo-600 text-indigo-600 bg-white' : 'text-gray-500 hover:text-gray-700' }}">
        <i class="ph-bold ph-file-text text-sm"></i>
        Leistungsberichte
        @if($leistungsberichte->count() > 0)
        <span class="text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-full">
            {{ $leistungsberichte->count() }}
        </span>
        @endif
    </a>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b">
            <tr>
                <th class="text-left px-5 py-3 font-semibold text-gray-600">Zeitraum</th>
                <th class="text-left px-5 py-3 font-semibold text-gray-600">Kunde</th>
                <th class="text-left px-5 py-3 font-semibold text-gray-600">Verknüpfte Rechnung</th>
                <th class="text-left px-5 py-3 font-semibold text-gray-600">Erstellt am</th>
                <th class="px-5 py-3"></th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
            @forelse($leistungsberichte as $lb)
            <tr class="hover:bg-gray-50">
                <td class="px-5 py-3 font-medium text-gray-800">
                    {{ $lb->date_from->format('d.m.Y') }} – {{ $lb->date_to->format('d.m.Y') }}
                </td>
                <td class="px-5 py-3 text-gray-700">{{ $lb->customer->name }}</td>
                <td class="px-5 py-3">
                    @if($lb->invoice)
                        <a href="{{ route('invoices.show', $lb->invoice) }}"
                           class="text-indigo-600 hover:underline font-mono text-xs">
                            {{ $lb->invoice->invoice_number }}
                        </a>
                    @else
                        <span class="text-gray-400 text-xs">–</span>
                    @endif
                </td>
                <td class="px-5 py-3 text-gray-400 text-xs">{{ $lb->created_at->format('d.m.Y') }}</td>
                <td class="px-5 py-3 text-right space-x-3">
                    <a href="{{ route('leistungsberichte.show', $lb) }}"
                       target="_blank"
                       class="text-gray-400 hover:text-indigo-600 text-xs">Anzeigen</a>
                    <form method="POST" action="{{ route('leistungsberichte.destroy', $lb) }}" class="inline"
                          onsubmit="return confirm('Leistungsbericht wirklich löschen?')">
                        @csrf @method('DELETE')
                        <button class="text-gray-400 hover:text-red-600 text-xs">Löschen</button>
                    </form>
                </td>
            ...
            <tr>
                <td colspan="5" class="px-5 py-10 text-center text-gray-400">
                    ...
                    <a href="{{ route('leistungsberichte.create') }}" class="text-indigo-600 hover:underline">
                        Ersten Leistungsbericht erstellen →
                    </a>
                </td>
            </tr>
            @endforelse
