    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="number" name="buffer_percent" x-model.number="bufferPercent"
                   value="{{ old('buffer_percent', $quote->buffer_percent) }}" min="0" max="100" step="1"
                   class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
        </div>
    </div>
    <div>
...
function quoteForm(existingFeatures) {
    return {
        ...
        bufferPercent: {{ old('buffer_percent', $quote->buffer_percent ?? 0) }},
        _counter:      existingFeatures ? existingFeatures.length : 0,

        get hourlyRateEffective() { return this.hourlyRate || {{ $defaultRate }}; },
        get rawHours()    { return this.features.reduce((s, f) => s + this.effectiveHours(f), 0); },
        get bufferHours() { return this.rawHours * ((this.bufferPercent || 0) / 100); },
        get totalHours()  { return this.rawHours + this.bufferHours; },
