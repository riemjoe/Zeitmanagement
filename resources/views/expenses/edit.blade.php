    ...
    <form method="POST" action="{{ route('expenses.update', $expense) }}" class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="category" value="{{ old('category', $expense->category) }}" list="expense-categories"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                <datalist id="expense-categories">
                    <option value="Fahrtkosten"><option value="Lizenz"><option value="Hardware">
                    <option value="Software"><option value="Unterkunft"><option value="Sonstiges">
                </datalist>
            </div>
            ...
            <div>
                ...
                <textarea name="notes" rows="2"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('notes', $expense->notes) }}</textarea>
            </div>
        </div>
